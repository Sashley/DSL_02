...
from app.models.shipping import Leg, Voyage, Port
from app import db
from sqlalchemy import or_, func
from app.utils.relationships import get_related_data, create_leg, update_leg, delete_leg

bp = Blueprint('leg', __name__)

# Define relationships for this model
relationships = [{'field_name': 'voyage_id', 'target_model': 'Voyage', 'relationship_field': 'voyage'}, {'field_name': 'port_id', 'target_model': 'Port', 'relationship_field': 'port'}]
...
def list_leg():
    ...
    if relationships:  # Only add joinedload if there are relationships
        query = query.options(*[
            db.joinedload(getattr(Leg, rel["relationship_field"]))
            for rel in relationships
        ])
    
    # Apply search filter if provided
    search_filter = get_search_filter(Leg, search)
    ...
    if request.headers.get('HX-Request'):
        return render_template('crud/leg/_rows.html', 
                            items=items,
                            has_more=pagination.has_next,
                            page=page)
    
    # For full page request, return complete template
    return render_template('crud/leg/list.html', 
                         items=items,
                         has_more=pagination.has_next,
                         page=page,
                         per_page=per_page)
...
def create_leg():
    if request.method == 'POST':
        try:
            success, item = create_leg(request.form)
            if success:
                return redirect(url_for("crud.leg.list_leg")) 
        except Exception as e:
            db.session.rollback()
            flash(f'Error: {str(e)}', 'error')
            return render_template('crud/leg/form.html', 
                                edit=False, 
                                form_action=url_for('crud.leg.create_leg')
                                , **get_related_data())
...
def edit_leg(id):
    ...
    if request.method == 'POST':
        try:
            if update_leg(item, request.form):
                return redirect(url_for("crud.leg.list_leg"))
        except Exception as e:
            db.session.rollback()
            flash(f'Error: {str(e)}', 'error')
            return render_template('crud/leg/form.html', 
                                edit=True, 
                                item=item,
                                form_action=url_for('crud.leg.edit_leg', id=id)
                                , **get_related_data())
