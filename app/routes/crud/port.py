...
from app.utils.relationships import get_related_data, create_port, update_port, delete_port
...
def list_port():
    ...
    if relationships:  # Only add joinedload if there are relationships
        query = query.options(*[
            db.joinedload(getattr(Port, rel["relationship_field"]))
            for rel in relationships
        ])
    
    # Apply search filter if provided
    search_filter = get_search_filter(Port, search)
    ...
    if request.headers.get('HX-Request'):
        return render_template('crud/port/_rows.html', 
                            items=items,
                            has_more=pagination.has_next,
                            page=page)
    
    # For full page request, return complete template
    return render_template('crud/port/list.html', 
                         items=items,
                         has_more=pagination.has_next,
                         page=page,
                         per_page=per_page)
...
def create_port():
    if request.method == 'POST':
        try:
            success, item = create_port(request.form)
            if success:
                return redirect(url_for("crud.port.list_port")) 
        except Exception as e:
            db.session.rollback()
            flash(f'Error: {str(e)}', 'error')
            return render_template('crud/port/form.html', 
                                edit=False, 
                                form_action=url_for('crud.port.create_port')
                                , **get_related_data())
...
def edit_port(id):
    ...
    if request.method == 'POST':
        try:
            if update_port(item, request.form):
                return redirect(url_for("crud.port.list_port"))
        except Exception as e:
            db.session.rollback()
            flash(f'Error: {str(e)}', 'error')
            return render_template('crud/port/form.html', 
                                edit=True, 
                                item=item,
                                form_action=url_for('crud.port.edit_port', id=id)
                                , **get_related_data())
