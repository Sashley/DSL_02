...
from app.models.shipping import Commodity
from app import db
...
def list_commodity():
    ...
    if relationships:  # Only add joinedload if there are relationships
        query = query.options(*[
            db.joinedload(getattr(Commodity, rel["relationship_field"]))
            for rel in relationships
        ])
    
    # Apply search filter if provided
    search_filter = get_search_filter(Commodity, search)
    ...
    if request.headers.get('HX-Request'):
        return render_template('crud/commodity/_rows.html', 
                            items=items,
                            has_more=pagination.has_next,
                            page=page)
    
    # For full page request, return complete template
    return render_template('crud/commodity/list.html', 
                         items=items,
                         has_more=pagination.has_next,
                         page=page,
                         per_page=per_page)
...
def create_commodity():
    if request.method == 'POST':
        try:
            ...
            return redirect(url_for("crud.commodity.list_commodity")) 
        except Exception as e:
            db.session.rollback()
            flash(f'Error: {str(e)}', 'error')
            return render_template('crud/commodity/form.html', 
                                edit=False, 
                                form_action=url_for('crud.commodity.create_commodity')
                                )
    ...
    if request.method == 'POST':
        try:
            ...
            return render_template('crud/commodity/form.html', 
                                edit=True, 
                                item=item,
                                form_action=url_for('crud.commodity.edit_commodity', id=id)
                                )
