...
from app.models.shipping import PackType
from app import db
...
def list_packtype():
    ...
    if relationships:  # Only add joinedload if there are relationships
        query = query.options(*[
            db.joinedload(getattr(PackType, rel["relationship_field"]))
            for rel in relationships
        ])
    
    # Apply search filter if provided
    search_filter = get_search_filter(PackType, search)
    ...
    if request.headers.get('HX-Request'):
        return render_template('crud/packtype/_rows.html', 
                            items=items,
                            has_more=pagination.has_next,
                            page=page)
    
    # For full page request, return complete template
    return render_template('crud/packtype/list.html', 
                         items=items,
                         has_more=pagination.has_next,
                         page=page,
                         per_page=per_page)
...
def create_packtype():
    if request.method == 'POST':
        try:
            ...
            return redirect(url_for("crud.packtype.list_packtype")) 
        except Exception as e:
            db.session.rollback()
            flash(f'Error: {str(e)}', 'error')
            return render_template('crud/packtype/form.html', 
                                edit=False, 
                                form_action=url_for('crud.packtype.create_packtype')
                                )
    ...
    if request.method == 'POST':
        try:
            ...
            return render_template('crud/packtype/form.html', 
                                edit=True, 
                                item=item,
                                form_action=url_for('crud.packtype.edit_packtype', id=id)
                                )
