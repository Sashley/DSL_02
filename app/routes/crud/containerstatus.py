...
from app.models.shipping import ContainerStatus
from app import db
...
def list_containerstatus():
    ...
    if relationships:  # Only add joinedload if there are relationships
        query = query.options(*[
            db.joinedload(getattr(ContainerStatus, rel["relationship_field"]))
            for rel in relationships
        ])
    
    # Apply search filter if provided
    search_filter = get_search_filter(ContainerStatus, search)
    ...
    if request.headers.get('HX-Request'):
        return render_template('crud/containerstatus/_rows.html', 
                            items=items,
                            has_more=pagination.has_next,
                            page=page)
    
    # For full page request, return complete template
    return render_template('crud/containerstatus/list.html', 
                         items=items,
                         has_more=pagination.has_next,
                         page=page,
                         per_page=per_page)
...
def create_containerstatus():
    if request.method == 'POST':
        try:
            ...
            return redirect(url_for("crud.containerstatus.list_containerstatus")) 
        except Exception as e:
            db.session.rollback()
            flash(f'Error: {str(e)}', 'error')
            return render_template('crud/containerstatus/form.html', 
                                edit=False, 
                                form_action=url_for('crud.containerstatus.create_containerstatus')
                                )
    ...
    if request.method == 'POST':
        try:
            ...
            return render_template('crud/containerstatus/form.html', 
                                edit=True, 
                                item=item,
                                form_action=url_for('crud.containerstatus.edit_containerstatus', id=id)
                                )
